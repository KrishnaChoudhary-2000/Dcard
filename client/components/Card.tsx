...
export const CardPreview: React.FC<{ data: ExecutiveData | null; onUpdate: (updates: Partial<ExecutiveData>) => void; }> = ({ data, onUpdate }) => {
    ...
    // Build a safe maps URL for the address when addressLink is empty or not a URL
    const toMapsUrl = (q: string) => `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(q)}`;
    const rawAddressLink = (data.addressLink || '').trim();
    let addressHref: string | undefined;
    if (rawAddressLink) {
      addressHref = /^https?:\/\//i.test(rawAddressLink) ? rawAddressLink : toMapsUrl(rawAddressLink);
    } else if (data.address) {
      addressHref = toMapsUrl(data.address);
    }
