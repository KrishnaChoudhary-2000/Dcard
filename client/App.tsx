...
const App: React.FC = () => {
    ...
    if (isSharedLink) {
        // Minimal backtracking prevention: push a state and trap back button
        useEffect(() => {
            const push = () => window.history.pushState(null, '', window.location.href);
            push();
            const handlePopState = (e: PopStateEvent) => {
                e.preventDefault?.();
                // Stay on current shared URL
                push();
            };
            window.addEventListener('popstate', handlePopState);
            return () => window.removeEventListener('popstate', handlePopState);
        }, [cardDataParam, shortIdParam, shortIdFromPath, sharedParam, cardIdParam]);
